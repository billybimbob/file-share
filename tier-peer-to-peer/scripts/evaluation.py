...
def dry_create_peers(num_peers: int, file_size: str):
...
async def run_cycle(num_peers: int, file_size: str, repeat: int, verbosity: int, dry_run: bool):
    """ Manages the creation, running, and killing of server and client procs """
    label = f'{num_peers}c{file_size}f'
    indexer_log = f'{LOGS}/{label}/indexer.log'

    if dry_run:
        dry_create_peers(num_peers, file_size)
        return
...
if __name__ == "__main__":
    ...
    args.add_argument("-d", "--dry-run", action="store_true", help="run peer init without running the actual peers")
